...
DRIVE_FILES = {
    'jobs_slim.csv':      '16zl9VSpzqU89vxwRQId3OOXguwLztPs7',  # UPDATE with slim CSV ID
    'job_embeddings.npy': '194QIDw9Kdr5OtpGTKctNB8tdloAkpR1S',
    'smart_practicum_results.csv': '1JdI0iYelpesaElQJ2S1lXLC7ytmFe6n2',
}
...
@st.cache_resource(show_spinner="Downloading job data (first time only)...")
def load_all_data():
    """Download data files from Google Drive and load them.
    Uses cache_resource so this runs exactly once, shared across all users.
    """
    import gdown

    data_dir = '/tmp/smart_practicum'
    ...
    min_sizes = {
        'jobs_slim.csv': 1_000_000,
        'job_embeddings.npy': 10_000_000,
        'smart_practicum_results.csv': 100_000,
    }

    for name, file_id in DRIVE_FILES.items():
        dest = paths[name]
        min_size = min_sizes.get(name, 0)
        file_ok = os.path.exists(dest) and os.path.getsize(dest) >= min_size
        if not file_ok:
            url = f'https://drive.google.com/uc?id={file_id}'
            gdown.download(url, dest, quiet=True, fuzzy=True)

    # Load job matching data
    jobs_df = pd.read_csv(paths['jobs_slim.csv'])
    jobs_df['required_skills'] = jobs_df['required_skills'].apply(
        lambda x: ast.literal_eval(x) if isinstance(x, str) and x.startswith('[') else []
    )
    job_embeddings = np.load(paths['job_embeddings.npy'], allow_pickle=True)

    # Load results data (for dashboard)
    results_df = None
    if os.path.exists(paths['smart_practicum_results.csv']):
        results_df = pd.read_csv(paths['smart_practicum_results.csv'])
