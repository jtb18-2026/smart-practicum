...
if 'bookmarked_jobs' not in st.session_state:
    st.session_state.bookmarked_jobs = set()   # set of j_idx values user bookmarked
    ...
    with col_right:
        ...
        if st.session_state.match_results is not None:
            ...
            for rank, j_idx in enumerate(display_idx, start=1):
                ...
                # Render insight box separately (avoids Streamlit HTML-in-f-string rendering issue)
                if insight:
                    st.markdown(insight, unsafe_allow_html=True)

                # Action buttons (Link / Compare / Save / Hide)
                btn_c1, btn_c2, btn_c3, btn_c4 = st.columns([2.4, 1.2, 1, 1])
                with btn_c1:
                ...
                with btn_c2:
                    ...
                    if st.button(cmp_label, key=f"cmp_{rank}_{j_idx}",
                        ...
                        st.rerun()
                with btn_c3:
                    is_bookmarked = int(j_idx) in st.session_state.bookmarked_jobs
                    bk_label = "✅ Saved" if is_bookmarked else "🔖 Save"
                    if st.button(bk_label, key=f"bk_{rank}_{j_idx}",
                                 help="Bookmark this job to review later"):
                        if is_bookmarked:
                            st.session_state.bookmarked_jobs.discard(int(j_idx))
                        else:
                            st.session_state.bookmarked_jobs.add(int(j_idx))
                        st.rerun()
                with btn_c4:
            ...
            if st.session_state.bookmarked_jobs:
                # Only show bookmarks that are still valid in the current filtered_df
                valid_bookmarks = [i for i in st.session_state.bookmarked_jobs
                                   if i < len(filtered_df)]
                if valid_bookmarks:
                    st.markdown("<div class='section-header'>🔖 Your Saved Jobs</div>",
                                unsafe_allow_html=True)
                    saved_rows = []
                    for j_idx_bk in sorted(valid_bookmarks):
                        jr = filtered_df.iloc[j_idx_bk]
                        sc = float(final_scores[j_idx_bk])
                        sal_bk = fmt_salary(jr) or 'Not listed'
                        url_bk = str(jr.get('job_posting_url', '') or '')
                        matched_bk = sorted(r_skills & job_skill_sets[j_idx_bk])
                        missing_bk = sorted(job_skill_sets[j_idx_bk] - r_skills)[:5]
                        matched_bk_html = "".join(f"<span class='skill-tag'>✓ {s}</span>" for s in matched_bk)
                        missing_bk_html = "".join(f"<span class='missing-tag'>✗ {s}</span>" for s in missing_bk)
                        link_html = (f'<a href="{url_bk}" target="_blank" '
                                     f'style="color:#1a56db; font-weight:600; text-decoration:none;">'
                                     f'View Job →</a>'
                                     if url_bk and url_bk != 'nan' else '')
                        st.markdown(f"""
                        <div class='match-card' style='border-left:4px solid #f59e0b;'>
                            <div style='display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:0.5rem;'>
                                <div>
                                    <div class='match-title'>{jr.get('title','Unknown')}</div>
                                    <div class='match-company'>
                                        {jr.get('company_name', jr.get('company',''))}
                                        {f"· 📍 {jr.get('location','')}" if jr.get('location') else ''}
                                    </div>
                                </div>
                                <span style='background:#fef3c7; color:#92400e; border:1px solid #fde68a;
                                             ...
                                             font-weight:600; white-space:nowrap;'>🔖 Saved</span>
                            </div>
                            <div style='display:flex; align-items:center; gap:0.8rem; margin:0.4rem 0;'>
                                <div class='score-bar-bg' style='flex:1;'>
                                    <div class='score-bar-fill' style='width:{int(sc*100)}%; background:{score_color(sc)};'></div>
                                </div>
                                <div style='font-family:Plus Jakarta Sans,sans-serif; font-weight:700;
                                            color:{score_color(sc)}; font-size:1.1rem; min-width:45px;'>{int(sc*100)}%</div>
                            </div>
                            <div style='font-size:0.85rem; color:#374151; margin:0.4rem 0;'>
                                💰 {sal_bk}
                                {"&nbsp;&nbsp;·&nbsp;&nbsp;" + link_html if link_html else ""}
                            </div>
                            {"<div style='margin-top:0.4rem;'>" + matched_bk_html + "</div>" if matched_bk else ""}
                            {"<div style='margin-top:0.3rem;'>" + missing_bk_html + "</div>" if missing_bk else ""}
                        </div>
                        """, unsafe_allow_html=True)
                        saved_rows.append({
                            'Title':          jr.get('title', ''),
                            ...
                            'Location':       jr.get('location', ''),
                            'Score_%':        int(sc * 100),
                            'Skills_%':       int(skill_scores[j_idx_bk] * 100),
                            'Education_%':    int(edu_scores[j_idx_bk] * 100),
                            'Semantic_%':     int(cos_scores[j_idx_bk] * 100),
                            'Salary':         sal_bk,
                            'Matched_Skills': ', '.join(matched_bk),
                            'Skills_To_Add':  ', '.join(missing_bk),
                            'Job_URL':        url_bk,
                        })
                    # Download saved jobs as CSV
                    if saved_rows:
                        saved_csv = pd.DataFrame(saved_rows).to_csv(index=False).encode('utf-8')
                        st.download_button(
                            "💾 Download Saved Jobs as CSV",
                            data=saved_csv,
