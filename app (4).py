...
# ── Auto-download data files from Hugging Face ────────────────────────────
JOBS_CSV    = '/tmp/jobs_cleaned.csv'
JOBS_EMB    = '/tmp/job_embeddings.npy'
RESULTS_CSV = '/tmp/smart_practicum_results.csv'

HF_REPO = 'jtb18/smart-practicum-data'

HF_FILES = {
    JOBS_CSV:    'jobs_cleaned.csv',
    JOBS_EMB:    'job_embeddings.npy',
    RESULTS_CSV: 'smart_practicum_results.csv',
}
...
def download_data_files():
    ...
    for dest, filename in HF_FILES.items():
...
def load_jobs_data(jobs_csv_path, jobs_emb_path):
    # Only load columns we actually need — saves ~400MB RAM
    needed_cols = ['title', 'company_name', 'location',
                   'edu_required', 'exp_required', 'required_skills']
    try:
        jobs_df = pd.read_csv(jobs_csv_path, usecols=needed_cols, encoding='utf-8')
    except Exception:
        try:
            jobs_df = pd.read_csv(jobs_csv_path, usecols=needed_cols, encoding='latin-1')
        except Exception:
            jobs_df = pd.read_csv(jobs_csv_path, encoding='latin-1')
    jobs_df['required_skills'] = jobs_df['required_skills'].apply(
        lambda x: ast.literal_eval(x) if isinstance(x, str) and x.startswith('[') else []
    )
    # Load embeddings as float16 to halve memory usage
    job_embeddings = np.load(jobs_emb_path).astype(np.float16)
